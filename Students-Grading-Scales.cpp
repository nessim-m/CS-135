...
int readFile(string[][2], int[][AMOUNT_OF_HW], int[][AMOUNT_OF_TESTS]);// declared function that returns the amount of students read from the file.
...
int findStudent(string[][2], int, string);// a declared function that compares the last string parameter with all the last names in the list of students.
...
int main()
{
	...
	numStudents = readFile(names, hw, exams);
	...
	while (search!= "Quit")
	{
		...
		index= findStudent(names, numStudents, search);
		...
		{
			outputStudentInfo(names[index][0], names[index][1], grade[index], hw[index], exams[index]);
		}
...
int  readFile( string names[][2], int hw[][AMOUNT_OF_HW], int exams[][AMOUNT_OF_TESTS])
{
	...
	inFile>> names[counter][0];

	while(!inFile.eof())
	{
		
		
		inFile>> names[counter][1];

		names[counter][0]= format(names[counter][0]);
		names[counter][1]= format(names[counter][1]);
...
int findStudent(string names[][2], int numStudents, string lastname)
{
	...
	for ( i=0; i < numStudents; i++)
	{
		...
		if (names[i][1].compare(lastname)==0)
		{
			found= true;
